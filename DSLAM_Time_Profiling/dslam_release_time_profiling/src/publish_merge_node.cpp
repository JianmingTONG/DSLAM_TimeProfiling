...
int main(int argc, char **argv)
{
    ...
    string str;
    str = PROFILING_PATH;
    str.append("publish_merge_node" + ss.str()+ ".txt");
  ...
  ros::Rate loop_rate1(0.5);   //定义发布的频率，0.5HZ
  loop_rate1.sleep();          //根据前面的定义的loop_rate,设置2s的暂停
  ros::Rate loop_rate(6);      //定义发布的频率，6HZ
  ...
  while (ros::ok() && tmp < frameend)   //循环发布msg
  {
    ...
    if ( tmp % 4 == 0 ){
        ...
        pub_info.publish(camera_info);//以1.5Hz的频率发布msg
...
//   }
    ...
    file << "publish_merge_node file takes " << time  << " seconds" <<endl;
    file.close();
#endif
