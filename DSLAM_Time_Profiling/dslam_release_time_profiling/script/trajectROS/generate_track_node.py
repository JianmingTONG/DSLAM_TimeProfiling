...
def callback(data):
    ...
    # br.sendTransformMessage(tf_msg)
    if(TIME_PROFILING):
        ofile = open(TIME_PROFILING_PATH + "generate_track_node_relpose.txt", 'a')
        end = time.time()
        ofile.write('generate_track_node_relpose time: %s Seconds'%(end-start))
        ofile.write('\n')
...
def callback_loop(data):
    ...
    NewLoop = True
    if(TIME_PROFILING):
        ofile = open(TIME_PROFILING_PATH + "generate_track_node_looppose.txt", 'a')
        end = time.time()
        ofile.write('generate_track_node_looppose time: %s Seconds'%(end-start))
        ofile.write('\n')
